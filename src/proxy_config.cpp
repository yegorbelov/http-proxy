...
int ProxyConfig::parse_int(const std::string &v, int def, int min_v,
                           int max_v) {
  if (v.empty())
    return def;
  char *end_ptr = nullptr;
  long n = std::strtol(v.c_str(), &end_ptr, 10);
  if (end_ptr == v.c_str() || n < min_v)
    return def;
  if (n > max_v)
    return (int)std::min<long>(n, max_v);
  return (int)n;
}

void ProxyConfig::parse_list_value(const std::string &s, char delim,
...
void ProxyConfig::apply_line(const std::string &line, size_t nline) {
  ...
  std::string value =
      (eq_pos + 1 < line.size()) ? line.substr(eq_pos + 1) : std::string{};
...
long long ProxyConfig::cacheMaxBytes() const {
  return (long long)cache_max_size_mb_ * 1024 * 1024;
}

long long ProxyConfig::cacheMaxEntryBytes() const {
  return (long long)cache_max_entry_size_mb_ * 1024 * 1024;
}
