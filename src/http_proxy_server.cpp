...
int HttpProxyServer::bind_listen() {
  ...
  a.sin_port = htons((uint16_t)ProxyConfig::instance().listenPort());
...
HttpProxyServer::HttpProxyServer()
    : filter_(ProxyConfig::instance().allowUrlPatterns(),
              ProxyConfig::instance().denyUrlPatterns()) {
  log_ = std::make_shared<EventLog>(ProxyConfig::instance().logPath());
  session_factory_ =
      std::make_unique<ClientSessionFactory>(filter_, cache_, log_);
  g_srv = this;
