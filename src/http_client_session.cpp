...
HttpClientSession::HttpClientSession(int client_fd, UrlFilter &f,
                                     ResponseCache &c,
...
void HttpClientSession::send_buffer(const std::string &s) {
  send_all(client_fd_, s.data(), s.size());
}
...
void HttpClientSession::process_parsed(const std::string &raw) {
  ...
  if (!UpstreamConnector::parse_http_proxy_target(first, head_block, fr, err)) {
    if (err.find("https") != std::string::npos) {
      send_text(501, "HTTPS/CONNECT: not implemented in this build (http only "
                     "on port 80).");
    } else
      send_text(400, err);
    return;
  ...
  const long long maxB = ProxyConfig::instance().cacheMaxEntryBytes();
  const std::string cache_key = "GET " + url;

  if (fr.method == "GET") {
    if (const auto *h = cache_.lookup(cache_key)) {
      if (log_)
        log_->cache_hit(cache_key);
      std::ostringstream o;
  ...
  if (orig_method == "GET" && res.status == 200 && res.ok) {
    bool ok_cache = true;
    if (const auto it = res.response_headers_lower.find("cache-control");
        it != res.response_headers_lower.end()) {
    ...
    if ((long long)res.body.size() > maxB) {
      ok_cache = false;
    }
    if (ok_cache) {
      CacheEntry e;
