...
class ProxyConfig {
   public:
    ...
    std::int64_t cacheMaxBytes() const;
    std::int64_t cacheMaxEntryBytes() const;
    ...
    const std::vector<std::string>& allowUrlPatterns() const { return allow_url_patterns_; }
    const std::vector<std::string>& denyUrlPatterns() const { return deny_url_patterns_; }

   private:
    ProxyConfig() = default;
    void set_defaults();
    void apply_line(const std::string& line, std::size_t nline);
    void parse_list_value(const std::string& s, char delim, std::vector<std::string>& out);
